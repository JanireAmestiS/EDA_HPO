...
def cost_function(solution: np.array, contador=contador, variables = variables, psb = psb):
    ...
    if contador.generacion != 0:
        ...
        if contador.generacion <3:
            quantl = np.quantile(quantl, 0.3)
        elif contador.generacion <6:
            quantl = np.quantile(quantl, 0.2)
        elif contador.generacion <9:
            quantl = np.quantile(quantl, 0.1)
        else:
            quantl = np.quantile(quantl, 0.05)
